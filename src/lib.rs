...
impl Point {
	...
	pub fn dot(&self, other: &Point) -> Scalar {
		// since e0 squares to zero, we are only left with e12 here
		self.e12 * other.e12
	}
...
impl Line {
	...
	/// inner product (dot product)
	pub fn dot(&self, other: &Line) -> Scalar {
		// NOTE: e0 * e0 is always zero
		self.e1 * other.e1 + self.e2 * other.e2
	}

	// /// inner product (dot product)
	// pub fn dot(&self, other: &Point) -> Scalar {
	// 	self.e0.dot(other.e20) + self.e0.dot(other.e20)
	// }
